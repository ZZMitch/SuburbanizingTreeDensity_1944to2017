...
drivers <- read.csv("Driver Influence/Rur_drivers.csv")
treecounts <- read.csv("Tree Time Series/Tree Counts/Rur_treecount.csv")
treechanges <- read.csv("Tree Time Series/Tree Change/Rur_treechange.csv")
...
# Manual check (10+) #
dr_building$TreeChange[16] = -1
dr_building$TreeChange[17] = -2
...
for(i in 3:14){ # For all timesteps...
  for(j in 1:nrow(drivers)) #For all driver squares...
    if(drivers[j,i] == "Recreational construction")
...
for(i in 3:14){
  golf_yr <- c(golf_yr, sum(drivers[,i] == "Recreational construction"))
}
golf_yr
...
# Fill in Driver Column #
dr_golf$Driver <- "Recreational construction"

# Manual check (10+) #
dr_golf$TreeChange[1] = -1
...
# Manual check (10+) #
dr_forest_exp$TreeChange[9] =  13
dr_forest_exp$TreeChange[13:14] =  1
...
# Manual check (10+) #
dr_tree_gr$TreeChange[9] = 5
dr_tree_gr$TreeChange[28] = 10
dr_tree_gr$TreeChange[58] = 15
dr_tree_gr$TreeChange[61] = 18
dr_tree_gr$TreeChange[63:64] = 18
...
# Manual check (10+) #
dr_plant_nat$TreeChange[3:4] = 1
dr_plant_nat$TreeChange[12] = 5
...
write.csv(dr_treechange_hum, "Rur_dr_tc_hum.csv")
write.csv(dr_treechange_nat, "Rur_dr_tc_nat.csv")
...
axis(1, labels = c("Building\nConstruction", "Land\nClearing", "Excavation", 
                   ...
                   "Sport\nConstruction", "Waterbody\nConstruction"), at = 1:9, tick = FALSE, cex.axis = 0.7)
...
axis(1, labels = c("Forest\nExpansion", "Other\nGreening", "Plantation\nPlanted", 
                   "Plantation\nNaturalized", "River\nInundation", "Tree\nRegeneration", "Tree\nRemoval", 
                   "Trees\nPlanted"), at = 1:8, tick = FALSE, cex.axis = 0.7)
