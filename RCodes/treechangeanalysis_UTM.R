...
drivers <- read.csv("Driver Influence/UTM_drivers.csv")
treecounts <- read.csv("Tree Time Series/Tree Counts/UTM_treecount.csv")
treechanges <- read.csv("Tree Time Series/Tree Change/UTM_treechange.csv")
...
write.csv(lcdata4417, file = "UTM_LC4417.csv")
...
# Manual check (10+) #
dr_road$TreeChange[3] = -1
dr_road$TreeChange[142] = -1
dr_road$TreeChange[144] = -1
dr_road$TreeChange[154] = -1
dr_road$TreeChange[201] = 2
dr_road$TreeChange[224:228] = -1
dr_road$TreeChange[294] = -5
dr_road$TreeChange[361] = -1
dr_road$TreeChange[438:439] = 1
dr_road$TreeChange[484] = -9
dr_road$TreeChange[486] = -5
dr_road$TreeChange[488] = -2
dr_road$TreeChange[517] = -20
dr_road$TreeChange[528] = -3
dr_road$TreeChange[532] = -2
dr_road$TreeChange[537] = -2
dr_road$TreeChange[591] = -1
...
# Manual check (10+) #
dr_minor$TreeChange[16] = 6
dr_minor$TreeChange[55] = -3
dr_minor$TreeChange[125] = -5
...
for(i in 3:14){ # For all timesteps...
  for(j in 1:nrow(drivers)) #For all driver squares...
    if(drivers[j,i] == "Plantation harvest")
...
for(i in 3:14){
  plant_har_yr <- c(plant_har_yr, sum(drivers[,i] == "Plantation harvest"))
}
plant_har_yr
...
# Fill in Driver Column #
dr_plant_har$Driver <- "Plantation harvest"

# Manual check (10+) #
dr_plant_har$TreeChange[22:24] = -1
...
# Manual check (10+) #
dr_forest_exp$TreeChange[3] = 1
dr_forest_exp$TreeChange[33] = 1
dr_forest_exp$TreeChange[35] = 3
dr_forest_exp$TreeChange[38] = 1
dr_forest_exp$TreeChange[48:49] = 2
dr_forest_exp$TreeChange[50:51] = 10
dr_forest_exp$TreeChange[55] = 3
dr_forest_exp$TreeChange[56] = 1
dr_forest_exp$TreeChange[69:70] = 3
dr_forest_exp$TreeChange[72] = 1
dr_forest_exp$TreeChange[91] = 1
dr_forest_exp$TreeChange[94] = 10
dr_forest_exp$TreeChange[108:109] = 2
dr_forest_exp$TreeChange[112] = 3
dr_forest_exp$TreeChange[124] = 16
dr_forest_exp$TreeChange[126] = 3
dr_forest_exp$TreeChange[137] = 3
dr_forest_exp$TreeChange[150] = 2
dr_forest_exp$TreeChange[159] = 1
dr_forest_exp$TreeChange[170] = 2
dr_forest_exp$TreeChange[184:187] = 2
...
for(i in 3:14){ # For all timesteps...
  for(j in 1:nrow(drivers)) #For all driver squares...
    if(drivers[j,i] == "Tree regeneration")
...
for(i in 3:14){
  tree_gr_yr <- c(tree_gr_yr, sum(drivers[,i] == "Tree regeneration"))
}
tree_gr_yr
...
# Fill in Driver Column #
dr_tree_gr$Driver <- "Tree regeneration"

# Manual check (10+) #
dr_tree_gr$TreeChange[31:32] = 3
dr_tree_gr$TreeChange[34:35] = 2
dr_tree_gr$TreeChange[53] = 3
dr_tree_gr$TreeChange[63] = 2
dr_tree_gr$TreeChange[69:71] = 1
dr_tree_gr$TreeChange[161] = 20
dr_tree_gr$TreeChange[184:185] = 8
dr_tree_gr$TreeChange[217] = 2
dr_tree_gr$TreeChange[224] = 2
dr_tree_gr$TreeChange[238] = 5
dr_tree_gr$TreeChange[241] = 1
...
for(i in 3:14){
  tree_ad_yr <- c(tree_ad_yr, sum(drivers[,i] == "Trees added"))
}
tree_ad_yr
...
for(i in 3:14){ # For all timesteps...
  for(j in 1:nrow(drivers)) #For all driver squares...
    if(drivers[j,i] == "Plantation naturalization")
...
for(i in 3:14){
  plant_nat_yr <- c(plant_nat_yr, sum(drivers[,i] == "Plantation naturalization"))
}
plant_nat_yr
...
# Fill in Driver Column #
dr_plant_nat$Driver <- "Plantation naturalization"

# Manual check (10+) #
dr_plant_nat$TreeChange[21:22] = -1
...
# Fill in TreeChange Column #
#plant_gr_yr$TreeChange <- plant_gr

# Manual check (10+) #

# Fill in Driver Column #
plant_gr_yr$Driver <- "Plantation grown"
#### Urban greening ####
urban_gr <- numeric(0)
...
for(i in 3:14){
  veg_gr_yr <- c(veg_gr_yr, sum(drivers[,i] == "Other greening"))
}
veg_gr_yr
...
# Manual check (10+) #
dr_veg_gr$TreeChange[33] = 7
dr_veg_gr$TreeChange[34] = 12
dr_veg_gr$TreeChange[43] = 1
...
write.csv(dr_treechange_hum, "UTM_dr_tc_hum.csv")
write.csv(dr_treechange_nat, "UTM_dr_tc_nat.csv")
...
axis(1, labels = c("Building\nConstruction", "Excavation", "Infrastructure\nRemoval", "Minor\nConstruction", 
                   "Plantation\nHarvested", "Road\nConstruction", "Sport\nConstruction", 
                   "Waterbody\nConstruction"), at = 1:8, tick = FALSE, cex.axis = 0.8)
