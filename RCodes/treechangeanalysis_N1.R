...
treecounts <- read.csv("Tree Time Series/Tree Counts/N1_treecount.csv")
...
write.csv(lcdata4417, file = "N1_LC4417.csv")
...
# Manual check (10+) #
dr_building$TreeChange[21] = -5
dr_building$TreeChange[55] = -5
...
# Manual check (10+) #
dr_road$TreeChange[23] = -15
dr_road$TreeChange[61] = -12
...
# Manual check (10+) #
dr_minor$TreeChange[18] = -8
dr_minor$TreeChange[26] = -3
...
for(i in 3:14){ # For all timesteps...
  for(j in 1:nrow(drivers)) #For all driver squares...
    if(drivers[j,i] == "Infrastructure removal")
...
for(i in 3:14){
  building_rm_yr <- c(building_rm_yr, sum(drivers[,i] == "Infrastructure removal"))
}
building_rm_yr
...
# Manual check (10+) #
dr_tree_ad$TreeChange[43] = 8
dr_tree_ad$TreeChange[73] = 7
...
# Manual check (10+) #
dr_tree_rm$TreeChange[7] = -15
dr_tree_rm$TreeChange[11] = -12
dr_tree_rm$TreeChange[13] = -7
dr_tree_rm$TreeChange[14:15] = -8
dr_tree_rm$TreeChange[18] = -1
dr_tree_rm$TreeChange[19:21] = -5
dr_tree_rm$TreeChange[27] = -11
dr_tree_rm$TreeChange[51] = -6
dr_tree_rm$TreeChange[56:57] = -7
dr_tree_rm$TreeChange[62] = -7
...
write.csv(dr_treechange_hum, "N1_dr_tc_hum.csv")
write.csv(dr_treechange_nat, "N1_dr_tc_nat.csv")
...
axis(1, labels = c("Forest\nExpansion", "Plantation\nPlanted", 
                   "Plantation\nNaturalized", "Tree\nRegeneration", "Tree\nRemoval", 
                   "Trees\nPlanted"), at = 1:6, tick = FALSE, cex.axis = 1)
